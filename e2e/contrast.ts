 ...
 *    `color-mix()`. This lab's surfaces are flat custom properties, but the
 *    tints laid over them are not: every `.wirecard` is
 *    `color-mix(in oklab, var(--accent) 4%, var(--surface))`, the hero aside is
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` over the page
 *    background, and the shared top bar mixes both its ink and its border out
 *    of `--accent`. Chromium hands `color-mix()` back to `getComputedStyle`
 *    unresolved, so axe files every node on one of those surfaces under
 *    `incomplete` rather than measuring it.
 ...
 *    on `violations` and nothing else, so it measured neither class: it never
 *    saw a `color-mix()` surface and never saw a faded one.
 *
 ...
 *    boxes are the `.hexdump-region` packet dumps — `overflow: auto` with
 *    `max-height: 20rem` around dumps that run to several hundred bytes, so
 *    most rows of a full ClientHello sit outside the box. Content scrolled past
 *    it is not dimmed or partly drawn — it is absent from the frame, and asking
 *    what colour it sits on has no answer. Its rect is still outside every
 *    ancestor's box, so the ancestor walk would find nothing behind it and fall
 *    through to white, inventing a failure. Skip it, and rely on the rows that
 *    ARE in view, which are measured for real.
 *
 ...
 * uses. Nothing here is transient: this lab's only timing is `src/ui/motion.ts`'s
 * `pause()`, which returns a resolved promise under `prefers-reduced-motion`,
 * so the staged observer reveal lands in one frame and every state the gate
 * scans stays on screen until the next click. The parameter is
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks several hundred-byte packet dumps — each one a
     * `<span class="hexrow">` per 16 bytes with a `<mark>` around every
     * highlighted nibble pair, and the splice stage highlights each of the
     * ~200 differing bytes individually — plus the field tables, the observer
     * lists and the verdict chips. Every one of those nodes re-walks the same
     * handful of ancestors up to `<body>`. Without the caches the pass re-reads the same
     ...
     * that is `.hexdump-region`, the `overflow: auto` / `max-height: 20rem`
     * frame around every packet dump on the page.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab's case is the
      // intro panel's `<details class="expert">` glossary, shut on arrival; the
      // gate opens it by clicking its `<summary>` — the route a reader has —
      // rather than setting `.open` from script, which is what the gate this
      // replaces did to every `<details>` on the page at once.
      // It also catches the observer panel's `custom hostname` input, which is
      // `hidden` until the select forks to it.
     ...
     * hamburger and GitHub marks, the `.chip-icon` verdict glyphs (✓ ✗ ⚠ ·),
     * the `.step-dot` ▸ and the observer's `.vis` eye/box glyphs — would be
     ...
     * attribute. Every one of this lab's `aria-hidden` marks is a glyph whose
     * meaning is also spelled out in adjacent real text — the chip's label, the
     * step's wording, and `.vis-word`'s literal " — readable" / " — opaque".
     */
     ...
     *
     * This lab has no SVG figure today — it builds its DOM through `src/ui/dom.ts`'s
     * `el()` helper, which only ever calls `document.createElement`, so it
     * cannot produce SVG at all without a deliberate change. The guard stays
     * because that change is exactly where the mistake happens and it is
     * silent: text placed in a `<g>` rather than a `<text>` is invisible on
     * screen, but a walk that measures it reports a 1:1 failure against the
